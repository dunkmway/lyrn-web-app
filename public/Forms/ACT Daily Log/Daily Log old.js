...
function openForm(id = undefined, view_type = undefined, element = undefined, pNumber = undefined, scaledScoresTest = undefined) {
  ...
  let forms = ["englishLessonsForm", "mathLessonsForm", "readingLessonsForm", "scienceLessonsForm"];
...
function scaledScoresTestsSetup() {
  ...
  for (let i = 0; i < tests.length; i++) {
    if (i != 0) {
      ele = createElements([['p']], [['testRow']], [['onclick']], [["openForm('scaledScores', undefined, undefined, undefined, '" + tests[i] + "')"]], [tests[i]], ['input-row-center', 'cursor'])
    }
    else {
      ele = createElements([['p']], [['testRow']], [['onclick']], [["openForm('scaledScores', undefined, undefined, undefined, '" + tests[i] + "')"]], [tests[i]], ['input-row-center', 'firstAnswer', 'cursor'])
    }
