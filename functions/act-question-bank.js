...
.document('/ACT-Questions/{questionID}')
.onCreate(async (snap, context) => {
  const questionData = snap.data();

  // check if the document is a question and if it has values in the topcs array
  if (questionData.topic.length == 0) { return };

    let id = await firebase.firestore().collection('ACT-Topics').where('topic', '==', questionData[topic]).get()

  //for each topics
  let data = {}
  questionData.topic.forEach(topic => {

    // Check isGrouped Flag

    // If Q is not grouped
       // ArrayUnion topic
    
    // isGrouped == True
       // Grab question bank topic document
       // Find other questions for this passage id
          // If none exists, add an array with just this question to the question bank
          // else Search arrays within the question bank for an array that has one of the other questions (search all questions) in the same passage 
             // add this question to the sub-array and reset document with the new sub-array
